...
def test_index():
    ...
    # Check that we get a 404 on the /
    resp =client.get("/")
    ...
    # make sure the default value work when POST has no data
    ...
    testdata = {'name': 'Julia', 'greet': 'Hola'}
    resp = client.post('/hello', data=testdata)
    assert_response(resp, contains='Julia') # the request should contain "Hola, Julia!"
